...
sf::FloatRect Structure::getGlobalBounds(){
	return this->renderRect.getGlobalBounds();
}
...
void Structure::render(sf::RenderTarget* target, sf::Shader* shader, const sf::Vector2f light_position){
	if (shader) {
		shader->setUniform("hasTexture", true);
		shader->setUniform("lightPos", light_position);
		target->draw(this->renderRect, shader);
	}
	else {
